...
Например, последовательность 0xA1 (0b10100001), упомянутая в описании выше, по правилам должна кодироваться как «0100010010101001», а реально используется «0100010010**0**01001» с одной единицей замененной нолем.
...
Теперь мы можем декодировать адреса и данные из дампа, рассчитать контрольные суммы по вышеприведенному алгоритму и сравнить полученное с каноническим [образом диска](agat-840-files/IKP_7A.DSK?raw=true) ИКП в формате DSK. Вот что получилось после декодирования первых двух секторов нулевой дорожки:
```
