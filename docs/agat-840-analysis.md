...
Каждая дорожка разбита на секторы, в формате 840Кб их 21. Каждый сектор содержит поле адреса, в котором указана метка тома, номер дорожки и номер сектора. И поле данных, состоящее из 256 байт данных + 1 байта контрольной суммы. Сектора, поля адреса и данных обычно разделены техническими промежутками, которые называются GAP.
...
*	SIDE ONE – равен 0 если идет чтение с 0 (нижней) стороны дискеты и 1 если чтение идет с 1 (верхней) стороны.
