...
В дисководе АГАТ 840Кб, как и во многих других стандартных дисководах, логическая единица соответствует низкому напряжению (0 В), логический ноль – высокому (5 В). 
> Хотя в дисководе АГАТ на 140Кб, клоне Shugart SA-400 - все наоборот.
...
> Что делать если данные начинаются с нуля, и мы не знаем значение предыдущего бита? 
> Такой ноль нужно кодировать в “00”, как будто перед ним была единица.

(Немного теории про устройство дискеты: дорожки, секторы, итд.)

## От теории к практике. 
Первое, что я сделал для того, чтобы понять, как информация хранится на дискете – снял полный дамп загрузки ИКП (http://agatcomp.ru/Apps/Ikp17.shtml). В графическом виде, он представлен ниже.
![ikp-dump]( agat-840-files/ikp-dump.png)

В левой части окна даны названия советующих сигналов на 34 контактном разъеме дисковода. В средней части мы видим waveform представление сигналов. 
Я кратко опишу основные:
*	INDEX – Сигнал от формируется в момент прохождения головки над первым сектором каждой дорожки. 
*	READ – импульсы данных с поверхности дискеты.
*	SIDE ONE – равен 1 если идет чтение с 0 (нижней) стороны дискеты и 0 если чтение идет с 1 (верхней) стороны.
*	DIRECTION – направление движения головок дисковода: 1 – от края к центру, 0 – от центра к краю.
*	STEP – при каждом единичном импульсе, головка должна перейти на соседнюю дорожку в направлении DIRECTION
*	READY – сигнал равен 0 если дисковод готов передавать данные.
*	TRACT 0 – сигнал равен 0 если головка дисковода находится на 0 (внешней) дорожке.
*	DRIVE EN 0 – при установке в 0 активирует первый дисковод на шлейфе.
Все сигнальные линии подтянуты к +5В, и по умолчанию имеют логический уровень 0.

Если мы немного приблизим сигнал данных, то сможем заметить следующие закономерности.
![ikp-dump-detail]( agat-840-files/ikp-dump-detail.png)
...
В программе логического анализатора, есть возможность экспортировать захваченные данные в файл формата CSV, где в первой колонке будет номер семпла, а во второй логический уровень (0/1). Причем для экономии, новая строчка добавляется только при изменении уровня.
...
Вот что из этого получилось для 0 и 1 в графическом виде:
<img width=600 src=agat-840-files/stats0.png></img><img width=250 src=agat-840-files/stats1.png></img>

На диаграммах наглядно видно, что все импульсы можно объединить в 4 группы:
*	ноль длительностью 0-30 — декодируем как 0
*	ноль длительностью 31-45 — декодируем как 00
*	ноль длительностью 46-… — декодируем как 000
*	один декодируем как 1 в любом случае

<table><th>Тип<td>Семплов<td>Микросекунд<td>Код</th>
...
Я задал в программе вышеуказанные цифры и получил на выходе поток данных в виде 0 и 1 закодированных в MFM. Пример:
```
...
Дальше мне пришлось обратится к интернету для поиска информации о логическом формате дискет АГАТ 840Кб, описание которого удалось найти в составе комплекта ПО за авторством kapitan-u, для преобразования образов дискет агата в внутренний формат эмулятора дисковода HxC. Также помогли исходники утилит преобразования dsk в nibble и информация на сайте agatcomp.ru.
Aghat 840k
```
...
Потратив немного времени на анализ содержимого файла в текстовом редакторе, я вычислил, что реальный desync для формата АГАТ 840Кб представляет собой «1000100100100100» или «0x12» в шестнадцатеричном виде. Следом за ним следует обязательный байт 0xFF, недоступный для ОС. 
...
### Приложения
[1]: agat-840-files/cd-8002-1.pdf
