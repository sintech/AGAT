...
<div align=right>*В поисках desync*</div>
...
Данные на дискетах записываются концентрическими дорожками вдоль направления вращения диска, в формате Агат 840Кб на дискету помещается по 80 дорожек с каждой стороны. Нумеруются они последовательно: четные дорожки с номерами: 0,2,4,6..158 на одной стороне, нечетные: 1,3,5,7..159 на другой. Всего 160 дорожек.

Каждая дорожка разбита на секторы, в формате 840Кб их 21. Каждый сектор содержит поле адреса, в котором указана метка тома, номер дорожки и номер сектора. И поле данных, состоящее из 256 байт данных + 1 байта контрольной суммы.
